...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get("http://localhost:3000/repeat/radioHead")
        .then(res => {
...
/*
    ...
    1: Send no queries on the URL -- what happened? "you sent an empty query" is logged in console.

    2: Send more than 1 query on the URL -- what happened? "you sent more than 1 query!" is logged in console.
*/

// CODE HERE

function myGetRequest() {
    axios.get("http://localhost:3000/query-test/?singer=thomyorke&bass=colingreenwood")
        .then((res) => {
            console.log(res.data)
        })
}
...
queryBtn.addEventListener("click", myGetRequest)
...
/*
    ...
    The key should be newFood (make sure to match the case and spelling exactly) and the value should be the 
    value of the food input. 
...
// CODE HERE 
const foodBtn = document.querySelector("#add-food-button")

function createFood() {
    console.log("blah")
    let foodInput = document.querySelector("#add-food-input")
    let body = {
